---
title: 'Travail en équipe'
category: 'human'
group: 'interpersonal'
level: 6
icon: '🤝'
order: 9
relatedProjects: ['syren-platform', 'luce-platform']
summary: "Collaboration efficace au sein d'équipes pluridisciplinaires pour atteindre des objectifs communs."
---

## Ma Définition

Le travail en équipe est la capacité à collaborer efficacement avec d'autres personnes pour atteindre des objectifs communs. Chez BH Technologies, je collabore quotidiennement avec d'autres développeurs, mais aussi avec les équipes métier, les UX designers, et les chefs de projet.

## Mes éléments de preuve

### Collaboration pluridisciplinaire sur Syren et Luce

Au sein de l'équipe SI de BH Technologies, je travaille avec des profils variés : développeurs frontend et backend, UX designer, chef de projet technique, et équipes métier. Cette diversité nécessite d'adapter sa communication et de comprendre les contraintes de chacun. J'ai appris à défendre mes choix techniques tout en restant ouvert aux feedbacks.

### Revues de code constructives

Les revues de code sont un moment clé de notre collaboration. Je les aborde avec un esprit constructif : l'objectif n'est pas de critiquer mais d'améliorer collectivement la qualité du code. J'apprécie autant de recevoir des feedbacks que d'en donner, car chaque review est une opportunité d'apprentissage. Je m'efforce de créer des PR de taille raisonnable, bien décrites, pour faciliter leur review.

### Mutualisation entre équipes Syren et Luce

Les plateformes Syren et Luce partagent des composants communs (authentification JWT, cartographie Mapbox, bibliothèques partagées). J'ai participé à la mutualisation de ces éléments, ce qui nécessitait une coordination étroite entre les deux équipes pour définir les interfaces et gérer les évolutions sans casser l'existant.

## Mon autocritique

Ma formation en DUT MMI était très orientée projets en équipe, ce qui m'a préparé à cette réalité professionnelle. J'ai appris à trouver ma place dans un collectif tout en apportant ma contribution technique.

Mon conseil : toujours communiquer proactivement sur les blocages. Une difficulté partagée tôt est souvent résolue plus vite qu'un problème gardé pour soi.

## Mon évolution dans cette compétence

Je souhaite développer mes compétences en pair programming et en mob programming, des pratiques que j'ai peu explorées. Je souhaite aussi améliorer ma capacité à onboarder de nouveaux membres dans une équipe.

## Réalisations associées

- [Plateforme Syren](/projects/syren-platform) - Collaboration équipe SI, ateliers utilisateurs
- [Plateforme Luce](/projects/luce-platform) - Coordination inter-équipes, mutualisation composants
